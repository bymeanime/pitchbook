...
// Protect seed POST route — only works with SEED_SECRET env var
const SEED_SECRET = process.env.SEED_SECRET

// ── Seed logic (shared between auto-seed GET and manual POST) ──
  ...
  for (const court of allCourtRecords) {
    ...
    for (let day = 0; day <= 6; day++) {
      for (let i = 0; i < timeSlots.length - 1; i++) {
        ...
        // Courts closed 13:00-14:00 for maintenance
        if (startTime === '13:00') {
          continue
        }
...
// ── POST: Manual seed with SEED_SECRET (force re-seed or additional data) ──
export async function POST(request: Request) {
  try {
    ...
    const stats = await runSeed()
    return NextResponse.json({ message: 'Database seeded successfully!', stats })
  } catch (error: unknown) {
